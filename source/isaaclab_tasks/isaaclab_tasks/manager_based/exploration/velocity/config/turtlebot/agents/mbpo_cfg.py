...
@configclass
class TurtlebotSACCfg:
    """SAC defaults for Turtlebot, aligned in scale with PPO configs.
    ...
    # training loop iterations for async v2 (monitor/log cycles)
    ...
    # ensemble dynamics
    num_networks: int = 2  # Minimal ensemble for more environments
    num_elites: int = 1
    # 动力学模型网络结构
    ...
    # rollout schedule
    rollout_batch_size: int = 1024  # Further reduced for better performance
    rollout_min_length: int = 1
    rollout_max_length: int = 8   # Further reduced for better performance
    ...
    use_cumulative_action: bool = True
    ...
    # notes
    notes: str = (
        "Use with: ./isaaclab.sh -p scripts/incremental_mbpo/train_isaac_async_v3.py "
        "--task=Isaac-Exploration-Rough-Turtlebot-v0 --device=cuda:0 --headless. "
